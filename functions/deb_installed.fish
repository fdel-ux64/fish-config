...
function __deb_installed_help
    ...
    echo "    deb_installed today --time"
    echo "    deb_installed -t days 3"
    echo "    deb_installed since 2026-07-01 --time"
    echo
    echo "OPTIONS:"
    ...
    echo "  deb_installed count this-week"
...
function deb_installed --description "List installed Debian/Ubuntu packages by install date with caching"
    ...
    set -l show_time 0
    set -l __deb_filtered_argv
    for a in $argv
        switch (string lower -- $a)
            case --time -t
                set show_time 1
            case '*'
                set -a __deb_filtered_argv $a
        end
    end
    set argv $__deb_filtered_argv
    ...
    if test $count_mode -eq 1
        ...
        set -l heading (__deb_period_label "$arg")
        if test -n "$on_date"
            set heading "$on_date"
        ...
        __display_packages "$show_time" "$cache_status" "$heading" $res
    end
end
