...
function deb_installed --description "List installed Debian/Ubuntu packages by install date with caching"
    ...
    # ---- since / until override (now actually applied) ----
    # FIX: previously since_epoch/until_epoch were set but s/e were never
    # updated from them. Now we write directly into s and e.
